-- Anthropic Claude LLM implementation for Samplanner
...
function M.is_ready()
  ...
  return true, "Anthropic provider ready"
end

-- Make HTTP request to Anthropic API using curl
-- @param messages: table - Array of message objects
-- @param system: string - System prompt
-- @param callback: function(response, error)
local function make_request(messages, system, callback)
    ...
    on_stdout = function(_, data)
      if data and data[1] and data[1] ~= "" then
        ...
        if ok and response then
          ...
          elseif response.content and response.content[1] then
    ...
    on_stderr = function(_, data)
      if data and data[1] and data[1] ~= "" then
        vim.schedule(function()
          vim.notify("Anthropic API stderr: " .. table.concat(data, "\n"), vim.log.levels.DEBUG)
        end)
      end
    end,
...
function M.parse_text(text, target_type, context, callback)
  ...
  local system_prompt = [[You are a task parsing assistant for a project planning tool.
  ...
  make_request(
    ...
    function(response, err)
      ...
      vim.schedule(function()
        ...
        elseif target_type == "structure" then
          -- Return raw parsed structure for further processing
          callback(parsed, nil)
        else
          callback(parsed, nil)
        end
...
function M.suggest_tags(task, existing_tags, callback)
  ...
  local system_prompt = [[You are a task tagging assistant.
...
llm.register_provider("anthropic", M)
